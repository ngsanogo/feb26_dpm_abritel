...
> Document créé le 2026-04-18, mis à jour le 2026-04-23.  
...
Nous avons choisi ce modèle (9B paramètres, 6.6 GB en quantifié) parce qu'il est suffisant pour de la classification de texte court tout en restant rapide en inférence locale. Qwen3.5 (Alibaba) est particulièrement bon en classification structurée avec sortie JSON.
...
Nous avons désactivé le raisonnement interne de qwen3.5 parce que ça rend le traitement ~18x plus rapide (mesuré en pratique : ~15 min vs ~4h sur 700+ avis). La tâche de classification ne bénéficie pas du chain-of-thought interne car le prompt fournit déjà tout le contexte nécessaire (catégories, descriptions, note, hint mots-clés). La qualité de classification n'est pas dégradée.

### Pourquoi 1 worker et 3 tentatives

Nous envoyons les requêtes Ollama une par une (séquentiel) parce qu'Ollama sérialise les inférences sur le GPU. Un benchmark sur 10 avis réels (3 runs par configuration) a montré qu'1, 2 et 4 workers donnent le même temps total (~20s pour 10 avis), confirmant que le parallélisme n'apporte aucun gain. Le mode séquentiel évite en plus les timeouts en cascade sur les longs runs (10 000+ avis) : avec plusieurs workers, les requêtes concurrentes s'empilent et certaines atteignent le timeout de 300s, multipliant le temps total par un facteur 5-10×. 3 tentatives avec backoff exponentiel (1s, 2s, 4s) parce qu'Ollama peut retourner une réponse mal formée sur le premier essai, surtout sur des avis courts ou ambigus.
...
| Workers Ollama | 1 | Séquentiel — Ollama sérialise sur le GPU, le parallélisme cause des timeouts sur les longs runs |
...
| Troncature prompt | 6000 chars | Sous les limites de contexte qwen3.5 |
