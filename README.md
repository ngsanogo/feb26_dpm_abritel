...
> **Depot public** : les CSV, JSON de meta, figures PNG et le rapport `data/analyse_problemes.md` **ne sont pas versionnes** (donnees regenerables). Clone puis suis la section **Regenerer les donnees** avant le notebook ou Power BI.

## Regenerer les donnees (ordre recommande)

1. **`uv sync`** puis **`uv run python 1_pipeline.py`** — cree `data/avis_enrichis.csv` et `data/releases.csv` (Abritel, 3 sources).
2. **`uv run python 1_benchmark.py`** — pour chaque marque, ecrit `data/benchmark/<marque>/avis_enrichis.csv`, `pipeline_meta.json` et `data/benchmark/benchmark_complet.csv` (scraping reseau, duree longue).
3. *(Optionnel)* **`uv run python 3_analyse_problemes.py`** — genere `data/analyse_problemes.md` a partir du benchmark (ou du fallback Abritel seul si pas d’etape 2).
4. Ouvre **`2_analyse.ipynb`** (kernel `.venv`) une fois les CSV presents ; les figures sous `data/figures/` sont produites par le notebook.
...
1. **Mots-cles** (deterministe, rapide) : 226 mots-cles FR/EN mappes aux 7 categories + Autre. Resultat dans `Catégorie_mots_cles`.
