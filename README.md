...
**Python** (scraping + filtrage heuristique) · **Grafana 11** (dashboards provisionnés en SQL) ·
...
```
        ...
        │                                                     │
        │   ↓ Grafana (3 dashboards SQL sur marts.*)          │
...
# 4. Grafana : http://localhost:3030  (admin / admin par défaut, surchargeable via .env)
#    → datasource « VoC Postgres » + 3 dashboards auto-provisionnés dans le folder « VoC » :
#       - VoC — Executive Overview
#       - VoC — Brand Comparison
#       - VoC — Operations & SAV
...
### Grafana (intégré au compose)

Un service **Grafana 11** est inclus dans `docker-compose.yml`, avec datasource Postgres et 3 dashboards auto-provisionnés :

- URL : **http://localhost:3030** — login par défaut `admin` / `admin` (surchargeable via `GRAFANA_ADMIN_USER` / `GRAFANA_ADMIN_PASSWORD` dans `.env`).
- Datasource **VoC Postgres** (uid `voc-postgres`) provisionnée via `grafana/provisioning/datasources/voc.yml`.
- Dashboards JSON sous `grafana/dashboards/` (folder « VoC » dans Grafana) :
  - **VoC — Executive Overview** : KPIs globaux (volume, note moyenne, % gravité haute, gap concurrents), évolution journalière par marque, distribution gravité, top catégories, table des avis critiques récents.
  - **VoC — Brand Comparison** : note moyenne par marque, volume × source (Trustpilot/GP/AS), % gravité haute par marque, catégories par marque (stacked), personas par marque (locataire/propriétaire), évolution rolling 7j de la note.
  - **VoC — Operations & SAV** : tickets ouverts, âge moyen, % avec réponse hôte, délai réponse, routage par équipe owner (sav/finance/produit/trust_safety), tickets par marque × source, time-series ouverture, table backlog top 30 du plus ancien.
- Auto-update : Grafana scanne `grafana/dashboards/` toutes les 30 s — modifie un JSON, recharge l'UI.

Toutes les requêtes sont en **SQL brut** sur `marts.fact_*` / `marts.dim_*` (pas de mart `dm_*` strictement nécessaire pour Grafana, ce qui laisse plus de flexibilité). Tu peux dupliquer/modifier les panels dans l'UI puis exporter le JSON pour le versionner.

Pour tout réinitialiser : `docker compose down -v` (efface aussi la metadata Airflow et les données VoC — à utiliser avec précaution).
...
│   └── tickets.csv
│
├── grafana/                    # Provisioning Grafana (datasource + dashboards)
│   ├── provisioning/
│   │   ├── datasources/voc.yml         # datasource Postgres VoC
│   │   └── dashboards/voc.yml          # loader dashboards JSON
│   └── dashboards/
│       ├── 01-executive-overview.json  # CODIR
│       ├── 02-brand-comparison.json    # benchmark Abritel/Airbnb/Booking
│       └── 03-operations-sav.json      # backlog tickets SAV
│
├── tests/                      # pytest (quality_filter + categorize + ollama + notion + slack)
├── docker-compose.yml          # postgres + airflow + grafana (+ ollama optionnel)
